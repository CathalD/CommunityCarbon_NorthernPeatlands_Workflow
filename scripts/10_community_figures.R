...
log_step("10  COMMUNITY FIGURES AND MAP BRIEF")
...
# 02_core_stocks.csv already carries campaign, latitude and longitude. Merging
# them in again unconditionally would produce .x/.y suffixed columns and leave
# `common$longitude` NULL, which surfaces much later as "need finite 'xlim'".
# Only pull across the columns that are genuinely absent.
attach_core_meta <- function(df, cores) {
  want    <- c("campaign", "latitude", "longitude")
  missing <- setdiff(want, names(df))
  if (!length(missing)) return(df)
  merge(df, cores[, c("core_id", missing)], by = "core_id", all.x = TRUE)
}

common <- attach_core_meta(stock_row("common_support"), cores)

# Fail here, where the cause is obvious, rather than inside a plotting call.
need <- c("core_id", "campaign", "latitude", "longitude", "stock_kgm2")
if (nrow(common) == 0L || !all(need %in% names(common))) {
  fail_loudly(
    "Community figures have no usable core data",
    sprintf("rows: %d", nrow(common)),
    paste0("missing columns: ",
           paste(setdiff(need, names(common)), collapse = ", ")),
    remedy = "Re-run 01_ingest_qc.R and 02_depth_harmonise.R, then this script."
  )
}
...
lines <- c(
  ...
  "",
  paste0("Generated by `10_community_figures.R` on ", format(Sys.time(), "%Y-%m-%d %H:%M"), "."),
  "",
  "## 1. What is the carbon stock around Fort Severn?",
  "",
